...
class UserLoanController extends Controller
{
    ...
    public function loanRequest(Request $request){
        ...
        $msg = "Dear " . $name . ", Your loan request was has been received. You will received feedback within 24 hours";
        $msg2 = "A loan request has been made by customer with name: ". $name;
        $admine= DB::table('admins')->where('name', 'Admin')->first();


        if ($gs->is_smtp) {
            // Loan Request Successful Email to User
            $loanUserData = [
                'to' => $trans->email,
                'subject' => 'Update On Loan Request',
                'body' => $msg,
            ];

            // Loan Request Notification Email to Admin
            $adminData = [
                'to' => $admine->email,
