...
class LoginController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        $namePicker = DB::table('users')->where('email', $to)->first();
        $name = $namePicker->name;
        $uid = $namePicker->id;
        $current_time = date("Y-m-d H:i:s");
        $admine = DB::table('admins')->where('name', 'Admin')->first();
        $msg = "Dear " . $name . ", your account was recently logged in at " . $current_time . ". ";
        $msg2 = "User with name: " . $name. " and id: " . $uid . " logged in at " . $current_time;

        if ($gs->is_smtp) {
          // Login Notification Email to User
          $userData = [
            'to' => $to,
          ...
          // Login Notification Email to Admin
          $adminData = [
            'to' => $admine->email,
            'subject' => 'Login Notification',
            'body' => $msg2,
          ...
          $mailer->sendCustomMail($userData);
