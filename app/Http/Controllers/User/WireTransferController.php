...
class WireTransferController extends Controller
{
    ...
    public function store(Request $request){
        ...
        $msg = "Dear " . $name . ", your Wire Transfer Request was sent Successfully ";
        $msg2 = "A wire transfer request has been made by customer with name: " . $name . " and email: " . $user->email;
        $admine = DB::table('admins')->where('name', 'Admin')->first();


        if ($gs->is_smtp) {
            // Wire Transfer Request Successful Email to User
            $WireTransferUserData = [
                'to' => $user->email,
                'subject' => 'Wire Transfer Request was successful',
                'body' => $msg,
            ];

            // Wire Transfer Request Notification Email to Admin
            $adminData = [
                'to' => $admine->email,
            ...
            // Send Loan Request Successful Email to User
            $mailer->sendCustomMail($WireTransferUserData);

            // Send Loan Request Notification Email to Admin
